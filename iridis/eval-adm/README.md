...
1. **PPL at 40k and 60k checkpoints** for both training versions (v1
   pre-B4 and v2 post-B4).
2. **Figure 4** (PPL vs MACs Pareto) for both versions.
3. **Figure 5 experiment matrix** -- 4 extraction variants x 2 versions
   x 2 checkpoints = 16 extractions, designed to isolate every
   methodological confounder in the router weight analysis.

This README contains the full methodology, running instructions,
results, and final interpretation. The `docs/reprod-notes.md` §C1
...
`adaptive_cotformer_..._single_final`:

| Tag | Checkpoint directory (leaf) | Training state |
|-----|-----------------------------|-----------------|
| v1 | `adm_lr0.001_bs8x16_seqlen256` | pre-B4 fix (original author's RNG-restore path commented out) |
| v2 | `adm_v2_lr0.001_bs8x16_seqlen256` | post-B4 fix (Gloo per-rank `all_gather` of CUDA RNG, full restore on resume) |

Both are 24-layer models with `n_repeat=5`, trained for 60,000 steps on
OWT2 with cosine LR schedule (peak 1e-3, `warmup_percent=0.2`,
`final_div_factor=0.05`), effective batch size 128, on 2x L4 24 GB via
DDP. See `docs/reprod-notes.md` §A6 for full training configuration and
§B4, §B9 for the RNG and checkpoint-save fix histories.

Each version is evaluated at both the intermediate **40,000-step** and
the final **60,000-step** checkpoints.

## Background: the Figure 5 bug and its critique

The original `get_router_weights.py` (commit `2723e30`) contains a
cascading bug (C1 in `docs/reprod-notes.md`): it computes element-wise
...
training versions and two checkpoints exercise each variant against the
actual model we want to analyse.
...
checkpoint directories exist, creates `run_N/adm_{v1,v2}/{exp4,exp5-*}/`
on the login node, and then submits a single sequential SLURM job to
`ecsstudents_l4` (1 GPU, 64 GB, 10 h wall, expected completion in
~5 h wall for the full matrix).
...
`$CKPT_DIR/eval_workspace/exp5-*/[40k|60k]/` on `/scratch`. They are
regeneratable and never copied to the Git-tracked `run_N/` tree.
...
All statistics are over `n = 8,135,168` validation tokens (3,973 batches
x 2,048 tokens per batch, one score per token per router). We report
router 4 (the last repeat) -- the quantity Figure 5 plots.

#### ADM v2 (post-B4 fix, primary result)
...
#### ADM v1 (pre-B4 training, control)
...
- `figure5_picascade.png` -- position-indexed cascade (C1 bug).
...
B4 RNG-restoration fix to the final ADM perplexity.
...
At 60k, v2 is 0.014 PPL lower than v1, which is well within the
width of either 95% CI (~0.76 PPL) and well below the inter-batch loss
SEM (~0.0081 in loss space, propagating to ~0.25 PPL). At 40k, the
delta is 0.006 PPL and in the opposite direction.
...
(SM89 vs SM80, CUDA 11.8 vs the authors' unrecorded CUDA), and seed
variance (the paper reports a mean of three seeds; we run one). None
is individually attributable without controlled experiments we cannot
run.
...
produce nearly identical router 4 distributions on L4/PyTorch 2.2. For
ADM v2 60k `exp5-original`, the router 4 mean under PI is 0.0336 and
under PT is 0.0345; relative delta 2.6%. At `exp5-cf-argparse`, the
relative delta is 2.7%. The C1 technical bug is therefore invisible on
our hardware, because `torch.topk(sorted=False, k=T)` returns
approximately the identity permutation, leaving the position index
effectively the same as the token index.
...
uses raw JSON dict loading (which leaves `dtype=None` and the autocast
path defaults to `float16`). Differences in router 4 mean between the
two variants are below 0.001 across all checkpoints and versions. The
dtype/autocast path is not a measurable confounder on our hardware.
...
(b) **Differences in the trained router itself.** Our ADM v2 reaches
    PPL 24.06 against the paper's 23.83 (delta +0.23). If Figure 5 was
    produced from a model trained with a different seed, different
    ...
    directly compare our raw router 4 distribution (mean 0.0424 at
    60k) against the authors'.
...
| Checkpoint | Router 4 mean (PT) | frac(score < 0.01) (PT) |
|------------|-------------------:|-------------------------:|
| 40,000 | 0.0220 | 44.9% |
| 60,000 | 0.0345 | 29.1% |

Router 4 mean increases by 57% between 40k and 60k, and the fraction
of tokens that essentially skip the final repeat decreases from 44.9%
to 29.1%. The direction and monotonicity match the paper's claim.
...
run_N/
  ...
  adm_v1/
    ...
    exp5-original/                      Experiment 1: cf, raw JSON config
      ...
      figure5_picascade.png             PI cascade histogram (C1 bug)
...
Intermediate `router_weights_{raw,picascade,ptcascade}.npy` files (~130
MB each) live on `/scratch` under
`$CKPT_DIR/eval_workspace/exp5-*/[40k|60k]/` and are **not** copied into
`run_N/`. They are regeneratable from `get_router_weights.py` if ever
needed.
...
- `docs/reprod-notes.md` §A6 -- ADM 60k PPL reproduction (target 23.83,
  ours 24.06). Details the delta attribution and the effect of B4
  being below the noise floor.
- `docs/reprod-notes.md` §B4 -- RNG state restoration on DDP resume,
  including the empirical v1 vs v2 magnitude update.
- `docs/reprod-notes.md` §B9 -- DDP checkpoint save failures and Gloo
  coordination fix history.
- `docs/reprod-notes.md` §C1 -- Figure 5 cascading bug and
  methodological critique (the Part C verdict that this document
  supports).
- `docs/reprod-notes.md` §C3 -- Orphan MoDBlock: 5 routers allocated,
  only 4 used (original author bug, fixed via B6).
- `get_router_weights.py` -- Unified extraction script with `--method`
  (`cf` / `cf-sorted` / `hooks`) and `--config-loading`
  (`raw` / `argparse`) flags driving the matrix.
- `plot_fig5.py` -- Figure 5 histogram plotter emitting
  `figure5_{raw,picascade,ptcascade}.png`.
